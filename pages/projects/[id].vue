<template>
    <div :class="[returnThemeClass(true, 'lime', mainTheme), 'min-h-screen']">
        <VimeoBase v-if="post?.res[0]?.videoUrl" :src="post?.res[0]?.videoUrl" isVimeo hidePause />
        <MediaBase v-else-if="post?.res[0]?.video?.url" :src="post?.res[0]?.video?.url" />
        <div class="md:p-4 p-2">
            ...
            <div class="flex justify-between items-center md:py-7 py-2">
                <div class="flex space-x-1 items-center h-10">
                    <div class="flex relative pl-10 pb-12">
                        <CategoryComponent v-for="(category, i) in post?.res[0]?.categories" :key="category?.id"
                            :title="category?.title" :i="i" />
                    </div>
                    <h2 v-for="category in post?.res[0]?.categories" :key="category?.id"
                        :class="[returnThemeClass(false, 'white', mainTheme), 'font-mono text-[10px] pl-2 md:text-xs uppercase']">
                        {{ category?.title }}
                    </h2>
                </div>
                <div>
                    <h4
                        :class="[returnThemeClass(false, 'white', mainTheme), 'font-mono text-[10px] md:text-xs uppercase']">
                        {{ post?.res[0]?.date }}</h4>
                </div>
            </div>
            <div class="md:grid md:grid-cols-2 flex flex-col">
                <div
                    :class="[returnThemeClass(false, 'darkblue', mainTheme), 'md:pr-10 formattedRichText helvetica-bold text-xl md:text-2xl md:!leading-6 !leading-none']">
                ...
                <div v-if="post?.res[0]?.mainSlider">
                    <div @click="nextSlide(post?.res[0]?.mainSlider?.length)"
                        class="flex h-[467px] mt-5 md:mt-1 lg:h-[800px] bg-cover bg-center rounded-[34px]" :style="{
            ...
            <div class="flex flex-col md:py-32 py-20">
                <h1
                    :class="[returnThemeClass(false, 'yellow', mainTheme), 'md:text-6xl helvetica-heavy text-[39px] pb-7 leading-8 text-center']">
                    {{ post?.res[0]?.quote }}
                </h1>
            ...
            <div class="flex flex-col md:flex-row md:flex-wrap">
                <div class="pb-2 md:p-4 md:w-1/2" v-for="image in post?.res[0]?.secondSlider" :key="image?.id">
                    <div class="flex h-[120] bg-cover bg-center rounded-[34px]" :style="{
            ...
            <div class="flex w-full flex-col">
                ...
                <div class="flex md:grid md:grid-cols-3 flex-col w-full">
                    <NuxtLink v-for="project in post?.similarProjects.slice(0, 3)" :key="project?.id"
                        :to="'/projects/' + project?.slug?.current" class="flex-1 flex-col my-4 md:mx-2 fadeIn">
                        <div class="flex h-80 bg-cover bg-center rounded-[34px] relative"
                            :style="{ backgroundImage: 'url(' + urlFor(project?.mainImage?.asset?._ref) + ')' }">
