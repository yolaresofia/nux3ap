...
// data is after 1 hour
const HOUR = 1000 * 60 * 60;
const anHourAgo = Date.now() - HOUR;
const lastCheck = new Date(check[0].created_at) < new Date(anHourAgo);
...
const { data: comment } = await useAsyncData("comment", async () => {
  const { data } = await clientSB.from("comment");
  return data;
});
</script>

<template>
  <div class="w-full bg-blue-200 fade" style="opacity: 0">
    <!-- section 1  -->
