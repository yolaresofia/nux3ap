<template>
    <div>
        <div>
            <div v-if="true" class="grid overflow-x-hidden">
                <!-- <div v-if="!store.settings.isUnderConstruction" class="grid overflow-x-hidden fadeIn"> -->
                <div id="load-ready" v-if="!loadReady"
                    class="h-screen w-screen opacity-100 transition-opacity duration-700 bg-primaryYellow absolute z-[100] inset-0 grid place-items-center">
                    <svg class="animate-pulse w-[80%] lg:w-6/12" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 128.53 40.46">
                        <g>
                            <path
                                d="M27.12,6.26l2.58,15.99h.08l2.67-15.99h7.54l-5.7,27.82h-7.83l-2.67-15.66h-.12l-2.67,15.66h-7.87L7.43,6.26h7.83l2.67,15.99h.12l2.5-15.99h6.58Z" />
                            <path
                            ...
                            <path
                                d="M81.55,27.53h-4l-3.29-8.62h-.08l.04,5.79v9.37h-7.5V6.26h8.33l4.46,11.04h.12l4.5-11.04h8.2v27.82h-7.5v-9.37l.04-5.79h-.08l-3.25,8.62Z" />
                            <path
            ...
            <div v-else class="flex-col relative h-screen bg-primaryYellow grid place-items-center">
                <div class="absolute top-0 left-0 w-full flex justify-center">
                    <div class="flex items-center py-2 justify-center">
                        <svg class="w-5/12 lg:w-3/12" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 128.53 40.46">
                            <g>
                                <path
                                    d="M27.12,6.26l2.58,15.99h.08l2.67-15.99h7.54l-5.7,27.82h-7.83l-2.67-15.66h-.12l-2.67,15.66h-7.87L7.43,6.26h7.83l2.67,15.99h.12l2.5-15.99h6.58Z" />
...
const loadReady = useState('loadReady', () => false)
...
onServerPrefetch(async () => {
    ...
    const res = query.result ? query.result[0] : {}
    store.settings = res
})
